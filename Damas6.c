...
#define COR_VERMELHA 12   // vermelho forte no console Windows
#define COR_AZUL     9    // azul claro
#define COR_PADRAO   7    // cor padrão (branco sobre preto)
...
void imprimirTabuleiro(struct tabuleiro x[8][8]) {
    ...
    for (i = 0; i < 8; i++) {
        printf("%d |", i);
        for (j = 0; j < 8; j++) {
            // Escolhe a cor de texto:
            if (x[i][j].status == 1) {
                if (x[i][j].cor == 0)      setColor(COR_VERMELHA, 0);
                else /* cor == 1 */        setColor(COR_AZUL, 0);
            }
            else {
                setColor(COR_PADRAO, 0);
            }

            // Imprime a peça ou ponto
            printf(" %c", x[i][j].show);

            // Restaura cor padrão para o próximo caractere
            setColor(COR_PADRAO, 0);
        }
        printf(" |\n");
    }
...
if(color == 0){//vermelho
    ...
    if(cdm1 - cd1 == 1 && (cdm2 - cd2 == 1 || cdm2 - cd2 == -1) && x[cdm1][cdm2].status == 0){
        ...
        //promoção para dama
        if ((color == 0 && cdm1 == 7) || (color == 1 && cdm1 == 0)) {
        x[cdm1][cdm2].show = 'd';
        x[cdm1][cdm2].tipo = 1;
        }
